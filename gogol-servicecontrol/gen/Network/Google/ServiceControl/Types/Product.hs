...
response =
  Response'
    {_rTime = Nothing, _rSize = Nothing, _rHeaders = Nothing, _rCode = Nothing}


-- | The timestamp when the \`destination\` service generates the first byte
-- of the response.
...
-- | The context of a span, attached to Exemplars in Distribution values
...
resourceInfo
    :: ResourceInfo
resourceInfo =
  ResourceInfo' {_riName = Nothing, _riType = Nothing, _riPermission = Nothing}
...
-- | An \'OK\' status allows the operation. Any other status indicates a
-- denial; [google.rpc.Status.details]() would contain additional details
-- about the denial.
