...
data CustomEventErrorCode
    ...
    | CEECInternal
      -- ^ @INTERNAL@
    | CEECPermissionDenied
      -- ^ @PERMISSION_DENIED@
    | CEECNotFound
      -- ^ @NOT_FOUND@
      deriving (Eq, Ord, Enum, Read, Show, Data, Typeable, Generic)

instance Hashable CustomEventErrorCode

instance FromHttpApiData CustomEventErrorCode where
    parseQueryParam = \case
        "UNKNOWN" -> Right CEECUnknown
        "INVALID_ARGUMENT" -> Right CEECInvalidArgument
        "INTERNAL" -> Right CEECInternal
        "PERMISSION_DENIED" -> Right CEECPermissionDenied
        "NOT_FOUND" -> Right CEECNotFound
        x -> Left ("Unable to parse CustomEventErrorCode from: " <> x)

instance ToHttpApiData CustomEventErrorCode where
    toQueryParam = \case
        CEECUnknown -> "UNKNOWN"
        CEECInvalidArgument -> "INVALID_ARGUMENT"
        CEECInternal -> "INTERNAL"
        CEECPermissionDenied -> "PERMISSION_DENIED"
        CEECNotFound -> "NOT_FOUND"

instance FromJSON CustomEventErrorCode where
    parseJSON = parseJSONText "CustomEventErrorCode"

instance ToJSON CustomEventErrorCode where
    toJSON = toJSONText
...
instance Hashable CustomEventEventType

instance FromHttpApiData CustomEventEventType where
    parseQueryParam = \case
        "UNKNOWN" -> Right CEETUnknown
        "INSERT" -> Right CEETInsert
        "ANNOTATE" -> Right CEETAnnotate
        x -> Left ("Unable to parse CustomEventEventType from: " <> x)

instance ToHttpApiData CustomEventEventType where
    toQueryParam = \case
        CEETUnknown -> "UNKNOWN"
        CEETInsert -> "INSERT"
        CEETAnnotate -> "ANNOTATE"

instance FromJSON CustomEventEventType where
    parseJSON = parseJSONText "CustomEventEventType"

instance ToJSON CustomEventEventType where
    toJSON = toJSONText
...
data CustomEventInsertInsertEventType
    = CEIIETUnknown
      -- ^ @UNKNOWN@
    | CEIIETImpression
      -- ^ @IMPRESSION@
...
instance Hashable CustomEventInsertInsertEventType

instance FromHttpApiData CustomEventInsertInsertEventType where
    parseQueryParam = \case
        "UNKNOWN" -> Right CEIIETUnknown
        "IMPRESSION" -> Right CEIIETImpression
        "CLICK" -> Right CEIIETClick
        x -> Left ("Unable to parse CustomEventInsertInsertEventType from: " <> x)

instance ToHttpApiData CustomEventInsertInsertEventType where
    toQueryParam = \case
        CEIIETUnknown -> "UNKNOWN"
        CEIIETImpression -> "IMPRESSION"
        CEIIETClick -> "CLICK"

instance FromJSON CustomEventInsertInsertEventType where
    parseJSON = parseJSONText "CustomEventInsertInsertEventType"

instance ToJSON CustomEventInsertInsertEventType where
    toJSON = toJSONText
