...
-- marked successful when the deletion is complete. Warning: Inserting
-- instances into a series while a delete operation is running for that
-- series could result in the new instances not appearing in search results
-- until the deletion operation finishes. For samples that show how to call
