...
-- listen to a Cloud Pub\/Sub topic, the adapter transmits the message when
-- a notification is received. This method also generates a response
-- containing an HL7v2 acknowledgement (\`ACK\`) message when successful or
-- a negative acknowledgement (\`NACK\`) message in case of error, suitable
-- for replying to HL7v2 interface systems that expect these
-- acknowledgements.
--
