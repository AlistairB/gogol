...
-- regardless of the enable_update_create setting on the FHIR store. The
...
-- grouping or ordering, but if the input data contains invalid references
-- or if some resources fail to be imported, the FHIR store might be left
-- in a state that violates referential integrity. The import process does
-- not trigger Pub\/Sub notification or BigQuery streaming update,
-- regardless of how those are configured on the FHIR store. If a resource
-- with the specified ID already exists, the most recent version of the
-- resource is overwritten without creating a new historical version,
-- regardless of the disable_resource_versioning setting on the FHIR store.
-- If transient failures occur during the import, it\'s possible that
-- successfully imported resources will be overwritten more than once. The
-- import operation is idempotent unless the input data contains multiple
-- valid resources with the same ID but different contents. In that case,
-- after the import completes, the store contains exactly one resource with
-- that ID but there is no ordering guarantee on which version of the
-- contents it will have. The operation result counters do not count
-- duplicate IDs as an error and count one success for each resource in the
-- input, which might result in a success count larger than the number of
-- resources in the FHIR store. This often occurs when importing data
...
-- referred to by many patients. If some resources fail to import, for
-- example due to parsing errors, successfully imported resources are not
-- rolled back. The location and format of the input data is specified by
-- the parameters in ImportResourcesRequest. Note that if no format is
...
-- ExecuteBundle, transaction bundles are not executed as a single
...
-- are also logged to Cloud Logging (see [Viewing
